...
EntityPlayer = EntityWorldEntity.extend({
	...
	attemptPlacement : function() {
		...
		if (this.placementBox.currentAnim != this.placementBox.anims.unplacable) {
			...
			if (ig.input.pressed('wall') && ig.game.wallCost.wood <= this.wood && ig.game.wallCost.metal <= this.metal) {
				ig.game.spawnEntity( EntityWall, this.placementBox.pos.x + 2, this.placementBox.pos.y + 8);
				this.wood -= ig.game.wallCost.wood;
				this.metal -= ig.game.wallCost.metal;

				snd = new ig.Sound( 'sounds/place.ogg' );
				snd.volume = 0.25;
				snd.play();
			}
			else if (ig.input.state('tower1') && ig.game.towerCost.wood <= this.wood && ig.game.towerCost.metal <= this.metal) {
				ig.game.spawnEntity( EntityTower1, this.placementBox.pos.x, this.placementBox.pos.y);
				this.wood -= ig.game.towerCost.wood;
				this.metal -= ig.game.towerCost.metal;

				snd = new ig.Sound( 'sounds/place.ogg' );
				snd.volume = 0.25;
				snd.play();
			}
			else if (ig.input.state('bomb') && ig.game.bombCost.wood <= this.wood && ig.game.bombCost.metal <= this.metal) {
				...
				this.wood -= ig.game.bombCost.wood;
				this.metal -= ig.game.bombCost.metal;
			}
			else if (ig.input.state('nuke') && ig.game.nukeCost.wood <= this.wood && ig.game.nukeCost.metal <= this.metal) {
				this.nuke();

				this.wood -= ig.game.nukeCost.wood;
				this.metal -= ig.game.nukeCost.metal;
